...
def generate_incremental_customers_data(data_creation_date, ammount_of_new_data = None):
    ammount_of_new_data = ammount_of_new_data if ammount_of_new_data else random.randint(int(CUSTOMERS_COUNT/TIME_SPAN_IN_DAYS - 50), int(CUSTOMERS_COUNT/TIME_SPAN_IN_DAYS))
    origin_data_path = os.path.join(CURRENT_DIRECTORY_PATH, ORIGINAL_JAFFLE_DATA_DIRECTORY_NAME, "raw_customers.csv")
    new_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_customers_training.csv")
    origin_headers, origin_data = split_csv_to_headers_and_data(csv_path=origin_data_path)
    headers, current_customers_data = split_csv_to_headers_and_data(csv_path=new_data_path)
    ...
    for customer_id in range(len(current_customers_data) + 1, ammount_of_new_data + len(current_customers_data) + 1):
...
def generate_incremental_orders_data(data_creation_date, ammount_of_new_data = None):
    ammount_of_new_data = ammount_of_new_data if ammount_of_new_data else random.randint(int(ORDERS_COUNT/TIME_SPAN_IN_DAYS - 50), int(ORDERS_COUNT/TIME_SPAN_IN_DAYS))
    customers_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_customers_training.csv")
    origin_data_path = os.path.join(CURRENT_DIRECTORY_PATH, ORIGINAL_JAFFLE_DATA_DIRECTORY_NAME, "raw_orders.csv")
    new_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_orders_training.csv")
    origin_headers, origin_data = split_csv_to_headers_and_data(csv_path=origin_data_path)
    headers, current_orders_data = split_csv_to_headers_and_data(csv_path=new_data_path)
    customers_headers, current_customers_data = split_csv_to_headers_and_data(csv_path=customers_data_path)
    all_order_statuses = list(set([row[3] for row in origin_data]))
    new_orders_data = [*current_orders_data]
    for order_id in range(len(current_orders_data) + 1, ammount_of_new_data + len(current_orders_data) + 1):
...
def generate_incremental_payments_data(data_creation_date, ammount_of_new_orders):
    origin_data_path = os.path.join(CURRENT_DIRECTORY_PATH, ORIGINAL_JAFFLE_DATA_DIRECTORY_NAME, "raw_payments.csv")
    new_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_payments_training.csv")
    orders_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_orders_training.csv")
    origin_headers, origin_data = split_csv_to_headers_and_data(csv_path=origin_data_path)
    headers, current_payments_data = split_csv_to_headers_and_data(csv_path=new_data_path)
    cyrrent_orders_headers, current_orders_data = split_csv_to_headers_and_data(csv_path=orders_data_path)
    ...
    for order_id in range(len(current_orders_data) - ammount_of_new_orders + 1, len(current_orders_data) + 1):
...
def generate_incremental_signups_data(data_creation_date, ammount_of_new_customers):
    new_data_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_signups_training.csv")
    headers = [
        "id",
        "user_id",
        "user_email",
    ...
    customers_csv_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_customers_training.csv")
    orders_csv_path = os.path.join(CURRENT_DIRECTORY_PATH, NEW_JAFFLE_TRAINING_DATA_DIRECORTY_RELATIVE_PATH, "raw_orders_training.csv")
    signups_headers, signups_data = split_csv_to_headers_and_data(csv_path=new_data_path)
